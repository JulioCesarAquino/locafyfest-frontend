...
import {
  ...
  Navigation, Trash2, Pencil, Plus, Map, Star, X,
...
function getAddressMapQuery(address: AddressEntry): string {
  const parts = [
    address.street,
    address.number,
    address.complement,
...
export default function Profile() {
  ...
  // ─── Address: Map links ───────────────────────────────────────────────────
  const openInGoogleMaps = (address: AddressEntry) => {
    const query = address.latitude && address.longitude
      ? `${address.latitude},${address.longitude}`
      : encodeURIComponent(getAddressMapQuery(address));
    window.open(`https://www.google.com/maps/search/?api=1&query=${query}`, '_blank');
  };

  const openInWaze = (address: AddressEntry) => {
    const url = address.latitude && address.longitude
      ? `https://waze.com/ul?ll=${address.latitude},${address.longitude}&navigate=yes`
      : `https://waze.com/ul?q=${encodeURIComponent(getAddressMapQuery(address))}`;
      ...
      <div className="space-y-6">
        ...
        <Tabs defaultValue="personal" className="space-y-6">
          <TabsList className="grid w-full grid-cols-2 md:grid-cols-4 h-auto gap-1">
            ...
            <TabsTrigger value="notifications" className="flex flex-col sm:flex-row items-center gap-1 sm:gap-2 py-2.5 text-xs sm:text-sm">
            ...
            <Card>
              ...
              <CardContent className="space-y-6">
                <div className="space-y-4">
                  {[
                    { key: 'email', label: 'Notificações por E-mail', desc: 'Receba atualizações sobre seus pedidos por e-mail' },
                    { key: 'whatsapp', label: 'Notificações no WhatsApp', desc: 'Receba lembretes e atualizações via WhatsApp' },
                    { key: 'sms', label: 'Notificações por SMS', desc: 'Receba lembretes importantes por SMS' },
                  ].map(({ key, label, desc }) => (
                ...
                <Button onClick={() => toast.success('Preferências salvas!')} className="flex items-center gap-2">
